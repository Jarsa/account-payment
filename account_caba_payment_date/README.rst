...
The module also makes the reconciliation work when the period of the
source documents is closed **through the tax lock date** (e.g. a credit
note applied to an invoice of an already-closed month, with no payment
involved). Standard Odoo ignores the tax lock date when choosing the
cash basis entry date, so the entry is dated inside the locked period
and posting is refused. With this module the tax lock date is taken into
account, so the standard fallback applies and the entry is dated on the
operation date (subject to the lock policy above). The same criterion is
applied to the exchange rate difference entry generated by the
reconciliation, and a traceability message is posted on the shifted
