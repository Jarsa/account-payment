...
class ResCompany(models.Model):
    ...
    def _add_business_days(self, start, days):
        """Date `days` business days after `start`, before it when negative.
        ...
        first = start + timedelta(days=1) if days > 0 else start
        begin = pytz.timezone(calendar.tz).localize(datetime.combine(first, time.min))
